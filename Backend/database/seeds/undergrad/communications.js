...
async function BachelorOfCommunications(num_of_students) {
    ...
    const Interests = ['History','Language', 'Art', 'Humanities', 'Writing', 'Communications', 'Music' ]
    const Core = ['https://www.handbook.uts.edu.au/directory/stm91104.html', 'https://handbook.uts.edu.au/directory/cbk91115.html']
    const Electives = ['https://www.handbook.uts.edu.au/directory/cbk91115.html', 'https://handbook.uts.edu.au/directory/cbk91119.html', 'https://handbook.uts.edu.au/directory/maj10046.html']
    ...
    for (let i = 0; i < (num_of_students); i++) {   
        ...
        if (year === 0) year = 1;
        studentService.setStudent(uniqid(), await buildStudents(year, 'Bachelor of Communications', Interests, Core, Electives))
    }
    // console.timeEnd("loop")

}

async function BachelorOfMusic(num_of_students) {
    ...
    const Interests = ['History','Language', 'Art', 'Humanities', 'Writing', 'Communications', 'Music' ]
    const Core = ['https://handbook.uts.edu.au/directory/maj10025.html', 'https://handbook.uts.edu.au/directory/stm90959.html']
    const Electives = ['https://handbook.uts.edu.au/directory/cbk91142.html']
    ...
    for (let i = 0; i < (num_of_students); i++) {   
        ...
        let year = Math.floor(Math.random() * 5) 
        if (year === 0) year = 1;
        studentService.setStudent(uniqid(), await buildStudents(year, 'Bachelor of Music', Interests, Core, Electives))
    }
    // console.timeEnd("loop")
