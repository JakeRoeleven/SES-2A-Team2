...
async function BachelorOfNursing(num_of_students) {
    ...
    const Interests = ['Education', 'Health', 'Hospitality', 'Fitness', 'Language']
    const Core = ['https://handbook.uts.edu.au/directory/stm90563.html']
    const Electives = ['https://handbook.uts.edu.au/directory/cbk91093.html']
    ...
    for (let i = 0; i < (num_of_students); i++) {   
        ...
        if (year === 0) year = 1;
        studentService.setStudent(uniqid(), await buildStudents(year, 'Bachelor of Nursing', Interests, Core, Electives))
    }
    // console.timeEnd("loop")

}

async function BachelorOfMidwifery(num_of_students) {
    ...
    const Interests = ['Education', 'Health', 'Hospitality', 'Fitness', 'Language']
    const Core = ['https://handbook.uts.edu.au/directory/stm90744.html', 'https://handbook.uts.edu.au/directory/stm90745.html']
    const Electives = ['https://handbook.uts.edu.au/directory/stm90746.html']
    ...
    for (let i = 0; i < (num_of_students); i++) {   
        ...
        let year = Math.floor(Math.random() * 5) 
        if (year === 0) year = 1;
        studentService.setStudent(uniqid(), await buildStudents(year, 'Bachelor of Midwifery', Interests, Core, Electives))
    }
    // console.timeEnd("loop")
...
BachelorOfNursing(400);
BachelorOfMidwifery(400);
